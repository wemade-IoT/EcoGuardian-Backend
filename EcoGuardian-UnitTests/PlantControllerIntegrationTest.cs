...
namespace EcoGuardian_UnitTests
{
    ...
    public class PlantControllerIntegrationTests
    {
        ...
        [Test]
        public async Task GetPlantsByUserId_ReturnsOk()
        {
        ...
        [Test]
        public async Task GetPlantById_NotFound()
        {
            _mockQueryService.Setup(s => s.Handle(It.IsAny<GetPlantByIdQuery>())).ReturnsAsync((Plant?)null);
            var result = await _controller.GetPlantById(99999) as NotFoundResult;
            Assert.That(result, Is.Not.Null);
            Assert.That(result.StatusCode, Is.EqualTo(404));
        }

        [Test]
        public async Task CreatePlant_ReturnsCreated()
        {
            var mockFile = new Mock<IFormFile>().Object;
            var resource = new CreatePlantResource(
                "IntegrationTestPlant", mockFile, "TestType", 10, 1, 0.5, 0.5, 0.5, false, 1
            );
